...
public class JKS {
    ...
    private final File keystoreDirSdcard;
    ...
    public JKS(@NonNull Context context, @NonNull String alias, @NonNull char[] password,
        ...
        this.keystoreDirSdcard = context.getExternalCacheDir();

        Log.i("tagg", new File(keystoreDirSdcard, "myKeyRing" + KEY_PEM_FILE_EXTENSION).getAbsolutePath());
        this.alias = alias;
    ...
    private void createKeystore() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                try {
                    keystore = generator.generateRoot(JKS.this);
                    File f = new File(keystoreDirSdcard, "myKeyRing"+KEY_STORE_FILE_EXTENSION);
                    os = new FileOutputStream(f);
                    keystore.store(os, password());
                    Certificate cert2 = keystore.getCertificate(alias());
                    sw = new FileWriter(new File(keystoreDirSdcard, "myKeyRing" + KEY_PEM_FILE_EXTENSION));
                    pw = new JcaPEMWriter(sw);
                    pw.writeObject(cert2);
                    pw.flush();
